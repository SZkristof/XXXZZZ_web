# Brandműhely — a főoldal teljes szövege

> **Ez a fájl generált.** A `npm run copy:doc` állítja elő a lefordított
> oldalból, tehát mindig azt mutatja, ami tényleg kint van. Ne szerkeszd
> közvetlenül — a következő futtatás felülírja. Ha szöveget akarsz
> változtatni, jelöld meg itt és szólj, vagy írd át a „forrás" alatt
> megnevezett fájlban.

Generálva: 2026-09-21

A rövid és a hosszú változat is szerepel ott, ahol két szöveg van:
a szolgáltatás-csempéken telefonon az egysoros jelenik meg, tablettől
felfelé a teljes leírás. Mindkettőt valakinek át kell tudnia nézni.

---

## 1. Hero — nyitóblokk

*Forrás: src/components/Hero.astro* · *Mód: semleges*

**Címke:** `EGYÉNI HIRDETÉSKEZELÉS OKTATÁS · ONLINE`

### H1 — Facebook és Google hirdetéskezelés oktatás személyesen

Szeretnéd végre önállóan, magabiztosan kezelni a hirdetéseidet? Az egyéni oktatás során gyakorlatiasan, vállalkozásodra szabva mutatom meg a hirdetéskezelés alapjait és működését.

Kampánybeállítástól a hatékony hirdetések létrehozásáig — lépésről lépésre, közösen!

**Gomb:** `Ingyenes 30 perces konzultáció` → /kapcsolat

**Gomb:** `Megnézem az árakat` → /arak

FIX árak. Rejtett költségek nélkül.

Szabó Kristóf

Facebook és Google szakértő · Oktató

---

## 2. Bizalmi sáv

*Forrás: src/components/SocialProof.astro + src/data/site.ts* · *Mód: képzés (terrakotta)*

Több, mint 200 magyar vállalkozás választotta már a Brandműhely oktatásait.

- 11 éve a szakmában
- Google és Meta minősítés
- Csak veled foglalkozok

**Címke:** `AKIKKEL MÁR DOLGOZTAM`

- Mizo
- Deutsche Telekom
- Rossmann
- Epson
- Sportfactory
- Football Factor

Továbbá: Heaven Laser & Beauty Szépségszalon, TippTour Utazási Iroda, Mokambo Kávé, Prémium Kőszőnyeg, Caninashop.hu, Trend Építészet Group, StoreFront, Hell Cuts Barber & more, AzsuzsA a magyartanár, GRK's Greek Kitchen, Deuter Magyarország, Store for Explorers és még sokan mások

---

## 3. Szolgáltatások

*Forrás: src/data/site.ts → services* · *Mód: semleges* · *Horgony: `#szolgaltatasok`*

**Címke:** `SZOLGÁLTATÁSOK`

### H2 — Valós megoldások vállalkozásodra szabva

Lépésről lépésre vezetlek végig a sikeres hirdetéskészítés folyamatain, hogy ne csak égesd a pénzt.

01

**Egyéni hirdetéskezelés oktatás**

Együtt építjük meg a kampányaidat, a te fiókodban.

Saját tempóban sajátíthatod el az otthonod kényelméből a hirdetéskezelés fortélyait. Megtanítalak rendszerben gondolkodni, miközben igazi kampányokat hozunk létre a vállalkozásod számára — az eredményeket pedig együtt vizsgáljuk meg utána.

Az oktatásról

02

**Teljes körű hirdetéskezelés, fiókmenedzsment**

Ha megbízható stratégiai partnert keresel.

Ha kiszerveznéd a marketinged, vagy megbízható partnert keresel, aki nem csak hirdetést kezel, hanem stratégiailag gondolkodik együtt a cégeddel — akkor valószínűleg egymást keressük.

Kampánykezelés

03

**Egyszeri kampány- és fiókbeállítás**

Gyors hirdetésindítás, akár 24 órán belül.

Rövid együttműködési lehetőség, ahol egyszeri fiók- és kampánybeállítást végzünk el a legjobb tudásunk szerint, az általad megadott adatok alapján.

Részletek

04

**Digitális marketing audit, fiókvizsgálat**

Átvizsgálom a fiókod, és megmondom, hol égeted el a pénzt.

Fiókvizsgálat, kampányelemzés, hatékonyságnövelési lehetőségek felkutatása és javaslattétel a jobb eredmények elérése érdekében.

Kérek auditot

05

**Mérések beállítása, audit**

GA4, GTM, mérés, feedek. Ingyenes hibafeltárással indulunk.

Google Analytics, Google Tag Manager, shopping feedek. Sejted, hogy rossz a mérésed, de nem tudod, mit csinálj pontosan? Hívj nyugodtan: ingyenes hibafeltárás után kapsz árajánlatot a javításra.

Hibafeltárás

06

**Online workshopok**

Élő és visszanézhető csoportos alkalmak, egy-egy témára.

Gyakorlatorientált, élő és visszanézhető csoportos alkalmak, ahol egy-egy témát és újdonságot dolgozunk fel a digitális marketing területéről.

Szólj, ha indul

---

## 4. Hirdetéskezelés egyéni oktatás

*Forrás: src/components/CourseIntro.astro* · *Mód: képzés (terrakotta)* · *Horgony: `#oktatas`*

### H2 — Hirdetéskezelés egyéni oktatás

**Címke:** `IDŐIGÉNY`

  kb. 5 óra egy felület

**Címke:** `MIKOR`

  hétköznap vagy hétvégén

**Címke:** `ELŐKÉPZETTSÉG`

  nem szükséges

**Gomb:** `Az oktatásról bővebben` → /kepzes

Eleged van abból, hogy nem mennek a hirdetéseid és csak égeted a pénzt? Ha nem tudod, hogyan kezdj hozzá, akkor az oktatásom pont Neked szól.

Megfelelő tudással vértezlek fel, hogy könnyedén boldogulj a Facebook, Instagram (Meta) vagy Google hirdetéskezelő felületén, és magabiztosan tudj kampányokat létrehozni a megfelelő célközönségnek. Az oktatás a saját tempód szerint zajlik, hétköznap vagy hétvégén: csupán 5 órát vesz igénybe egy felület elsajátítása, és nem igényel előképzettséget — de természetesen ha van, az sem baj.

Azonnali tudást kapsz, ahol nem csak általános sallangot hallasz a hirdetésekről, hanem közösen alkotunk, hogy felépítsük a digitális marketinged.

Nem tudod, mi lenne számodra a megfelelő? Hívj: +36 70 670 2600

---

## 5. Három lehetőség + összehasonlítás

*Forrás: src/components/Comparison.astro* · *Mód: semleges*

### H2 — Három lehetőséged van, nem mindegy mit választasz!

Az egyéni oktatás neked szól, ha a marketingesekben már nem bízol, és a csoportos tanfolyamok alatt rájöttél, hogy az nem neked való.

| Szempont | Marketinges · Kiszervezed | Csoportos tanfolyam · Videókat nézel | Egyéni oktatás · Megcsinálod, vezetéssel |
| --- | --- | --- | --- |
| Teljesítmény | Mástól függsz | Túl általános tudás | 100% egyénre szabva |
| Tulajdonjog | Gyakran vitás | A tiéd | A tiéd, örökre |
| Megbízhatóság | Napok, mire válaszol | Nincs kitől kérdezni | Azonnal tudsz kérdezni |
| Te vagy a központban? | Részben, + más cégek | Nem, általános | Igen, kizárólag |
| Havi díj | 150–400 e Ft havonta | Havi előfizetés 20–30 e Ft | Nincs havidíj. Egyszer fizetsz a tudásért |
| Elégedettség | Változó | Változó | 200+ pozitív visszajelzés |

---

## 6. Hogyan működik?

*Forrás: src/components/HowItWorks.astro* · *Mód: semleges* · *Horgony: `#hogyan`*

### H2 — Hogyan működik?

Nincs tananyag, nincs házifeladat, csak Te, az Oktatód és a vállalkozásod. Szuperintenzív marketing tanfolyam, 100% gyakorlattal, mellébeszélés nélkül.

**Címke:** `ALKALMANKÉNT`

  60 perc

**Címke:** `HETENTE MAX.`

  2 alkalom

**Címke:** `HELYSZÍN`

  Online (Google Meet) vagy személyesen Budapesten

**Címke:** `VISSZANÉZHETŐ`

  Igen — ingyenes felvétel

1

**Ingyenes 30 perces konzultáció**

Elmondod, mivel foglalkozol és hol tartasz. Megmondom, hogyan tudok segíteni.

2

**Irányválasztás**

Meta, Google vagy LinkedIn — és hány alkalom kell hozzá. Pont azt kapod, amire szükséged van. Nem többet, nem kevesebbet!

3

**Első óra: képernyőmegosztás, közös munka**

Nem diákat nézünk. 60 percen át megosztod a képernyőd, és együtt dolgozunk a saját hirdetési fiókodban, a saját termékeiden vagy szolgáltatásodon.

4

**Hirdetések indítása és optimalizálása**

Élesben indulnak a kampányok, nem tesztkörnyezetben. Aztán együtt optimalizáljuk őket, amíg futnak.

5

**Eredmények elemzése**

Átnézzük a számokat: mi hozott eredményt, mi nem, és mire megy el a büdzsé. Megtanulod, mit kell nézned hetente, és mikor kell hozzányúlni a hirdetéshez.

6

**Tanfolyam-felvételek — nem kell jegyzetelned**

Minden alkalomról videófelvétel készül, amit az óra után megkapsz. Bármikor visszanézheted, ingyen.

---

## 7. Platformok

*Forrás: src/components/Platforms.astro + src/data/site.ts → platforms* · *Mód: semleges* · *Horgony: `#platformok`*

**Címke:** `PLATFORMOK`

### H2 — Kezdésnek egyet választunk — vagy kettőt kombinálhatunk

Nem kell mindenhol ott lenned. Az első alkalmon eldöntjük, hol vannak a te vevőid, és arra koncentrálunk.

**Meta Ads**

Facebook & Instagram

Webshopok, szolgáltatások, helyi vállalkozások — itt szinte mindenkinek ajánlott hirdetni.

**Google Ads**

Keresés, Shopping, PMax

Keresési, vásárlási szándék elkapása vevőszerzés céljából.

**LinkedIn Ads**

B2B célzás

B2B, ahol pozíció, cégméret és iparág szerint kell célozni.

A 10 alkalmas csomagban két platform fér el — a leggyakoribb páros a Meta + Google.

---

## 8. Eredmények

*Forrás: src/components/Results.astro + src/data/proof.ts → results* · *Mód: semleges* · *Horgony: `#eredmenyek`*

**Címke:** `EREDMÉNYEK`

### H2 — Mások már megtanulták, te készen állsz?

Szeretnék megosztani Veled pár eredményt korábbi ügyfeleimtől, hogy lásd, mire számíthatsz.

Szolgáltatás · Fitness

Facebook egyéni oktatás

Új fizető vendég

10

15 nap alatt

19 űrlapkitöltésből. A hirdetést le kellett állítani, mert nem győzte fogadni a jelentkezőket.

Szolgáltatás · Szépségipar

Költség/vásárlás

3 500 Ft

1 800 Ft

2 hónap alatt

Az Advantage+ kikapcsolásával és konkrét célzási stratégiákkal sikerült javítani a vásárlásonkénti költséget.

Webshop · Sport

Facebook és Google hirdetéskezelés

Black Friday bevétel

18 millió Ft

1 nap alatt

Black Friday és karácsonyi akciók kampánykezelésére kértek fel, ahol 1 nap alatt értük el a közel teljes évi bevételt.

Oktatás

Költség / lead

40 Ft

napi 700 Ft keretből

Kecskeméti ügyfelemnek napi 700 forintból kellett hírlevél-feliratkozókat szereznünk egy „csali" e-mail kampányhoz.

- **képzett vállalkozás**
  200+ képzett vállalkozás

- **szakmai tapasztalat**
  11 év szakmai tapasztalat

- **platform: Meta, Google, LinkedIn**
  3 platform: Meta, Google, LinkedIn

- **elégedettség**
  100% elégedettség

---

## 9. Vélemények

*Forrás: src/components/Testimonials.astro + src/data/proof.ts → testimonials* · *Mód: semleges* · *Horgony: `#velemenyek`*

**Címke:** `VÉLEMÉNYEK`

### H2 — Ügyfél visszajelzések

Azzal kerestem meg őket, hogy segítsenek végre átlátni, hogy a jelenlegi hirdetéseink valóban jól működnek-e. Kristóf nagyon részletesen végigment a marketingünkön — és ami számomra különösen hasznos volt: nemcsak elmondta, mit kellene másképp csinálnunk, hanem együtt át is szerkesztettük és optimalizáltuk a kampányokat. Nem általános marketingelmélet volt, hanem kifejezetten a saját vállalkozásunkra szabott, gyakorlatias segítség.

DB Demeter B. tulajdonos · Szépségipar EGYÉNI OKTATÁS

Korábban az volt a problémám, hogy szerettem volna több új vendéget szerezni, de nem voltam biztos benne, hogyan érdemes Facebookon hirdetnem. 15 nap után 19 ember töltötte ki az űrlapot, közülük 12-vel már beszéltem, és 10-en azóta a vendégeim lettek. Most egy kicsit le is kellett állítanom a hirdetést, mert annyi új jelentkező érkezett, hogy nem győztem őket fogadni.

NT Németh T. tulajdonos · Fitness EGYÉNI OKTATÁS

---

## 10. Árak

*Forrás: src/components/Pricing.astro + src/data/site.ts → packages* · *Mód: képzés (terrakotta)* · *Horgony: `#arak`*

**Címke:** `ÁRAK`

### H2 — FIX ÁRAK — REJTETT KÖLTSÉGEK NÉLKÜL

Nincs havidíj, nincs hűségidő, nincs rejtett felár. Ráadásul ingyen felvétel jár a teljes oktatásra.

AUDIT, FIÓKVIZSGÁLAT

**Egy alkalom**

Konkrét probléma megoldása, kampány indítása

35 000 Ft

1x 60 perc

Kinek jó: Egy konkrét kérdés, egy elakadás, vagy egy alapos fiók-átnézés.

**Gomb:** `Ez érdekel` → /kapcsolat

START CSOMAG

**5 alkalmas csomag**

4+1

A legnépszerűbb.

140 000 Ft

28 000 Ft / alkalom · 5 alkalom összesen

Megspórolsz 35 000 Ft-ot

4 alkalmat fizetsz, 5-öt kapsz.

Kinek jó: Nulláról felépítjük a hirdetéseidet Facebookon vagy Google-n, rendet rakunk a meglévő, gyengén teljesítő fiókodban.

**Gomb:** `Ezt választom` → /kapcsolat

TELJES CSOMAG

**10 alkalmas csomag**

7+3

Két platform, a teljes út.

245 000 Ft

24 500 Ft / alkalom · 10 alkalom

Megspórolsz 105 000 Ft-ot

7 alkalmat fizetsz, 10-et kapsz.

Kinek jó: Érdekel a Facebook és Google hirdetéskezelés világa. Komolyan skálázni akarsz, és két csatornát futtatnál párhuzamosan.

**Gomb:** `Ezt szeretném` → /kapcsolat

Egy alkalom listaára 35 000 Ft. A csomagoknál ehhez képest kapsz ingyen alkalmakat.

---

## 11. Gyakori kérdések

*Forrás: src/data/proof.ts → faqs* · *Mód: semleges* · *Horgony: `#gyik`*

**Címke:** `GYAKORI KÉRDÉSEK`

### H2 — Amit a legtöbben megkérdeznek

Ha nincs köztük a tiéd, írj vagy hívj — és mindent átbeszélünk.

**K: Még soha nem hirdettem. Így is jelentkezhetek?**

Persze. Az ügyfeleim többsége soha nem állított be hirdetést. Együtt kattintunk végig mindent, és ha valami nem világos, átbeszéljük — nincs buta kérdés. Ha már hirdettél, akkor a szintednek megfelelően haladunk a hirdetéskezelésben, és komplexebb módszereket mutatok.

**K: Havi 30–50 ezer forintot tudok hirdetésre költeni. Ez elég?**

Az ügyfeleim jelentős része ebben a sávban van, és pont itt számít a legtöbbet, hogy jó helyre menjen a pénz. Kis kerettel nincs mozgástér a hibázásra — ezért éri meg megtanulni jól hirdetni.

**K: Miben más ez, mint egy online kurzus?**

Egy csoportos online kurzus általánosságban tanít meg az alapokra. Az én egyéni oktatásaimon a te hirdetési fiókodat nyitjuk meg, a te termékeidet és szolgáltatásaidat nézzük át, és a te kereteddel állítunk be hirdetéseket. Ha elakadsz, azonnal tudsz kérdezni, nem egy Facebook-csoportban kapsz választ három nap múlva.

**K: Mi van, ha a végén mégis inkább kiszerveznénk?**

Akkor is jobban jársz, mintha most szerveznéd ki: tudni fogod, mit kérsz számon. És ha tényleg ez a jó döntés, szólj — természetesen ezzel is foglalkozunk.

**K: Mennyi idő alatt végzünk az 5 alkalommal?**

Jellemzően 5 hét, heti egy alkalommal. Ha sietsz, heti kettővel 2–3 hét alatt is megtanulhatod a hirdetéskezelést. Ennél sűrűbben nem javaslom — a két alkalom között érdemes pihenni, mert elég intenzívek az órák.

**K: Kapok felvételt az alkalmakról?**

Igen, mindegyikről, felár nélkül. Az alkalom végén, legkésőbb másnap kiküldöm e-mailben. Nem kell jegyzetelned, és bármikor visszanézheted.

---

## 12. A másik fele — Ügynökség

*Forrás: src/components/AgencyHalf.astro + src/data/site.ts → agencyServices* · *Mód: ügynökség (kék)* · *Horgony: `#ugynokseg`*

**Címke:** `HIRDETÉSKEZELÉS KISZERVEZÉSE`

### H2 — És ha nincs rá időm, hogy magamnak csináljam a hirdetéseim?

Van, akinek egyszerűen nincs rá kapacitása — vagy elmúlt a bizalom a jelenlegi marketingesével. Beszélgessünk, és nézzük meg, tudok-e segíteni és átvenni a hirdetéskezelést.

Magas színvonalú szakmai háttér, őszinteség és stratégiai megbeszélések. Nálam a marketing nem csak a kivitelezésről szól.

**Gomb:** `Hirdetéskezelésről bővebben` → /ugynokseg

**Gomb:** `Ajánlatot kérek` → /kapcsolat

**Kampánykezelés**

Átveszem a Meta, Google vagy LinkedIn kampányaidat: tervezés, stratégia, felépítés, mérés, optimalizálás, heti/havi riport. A fiók végig a tiéd marad, és bármikor belenézhetsz.

**Webfejlesztés**

Gyors, mérhető weboldal vagy landing oldal, ami a hirdetéseidhez készül — nem sablonból. A sebesség közvetlenül olcsóbbá teszi a kattintásaidat.

**Egyszeri kampány- és fiókbeállítás**

Rövid együttműködés: 24–48 órán belül felépítem az első kampányokat az általad megadott adatok alapján, aztán a tiéd. Nincs havidíj és nincs folytatási kötelezettség.

**Digitális marketing audit, fiókvizsgálat**

Fiókvizsgálat, kampányelemzés és hatékonyságnövelési javaslatok. Megkapod írásban, mit érdemes átalakítani — akkor is, ha utána egyedül csinálod tovább.

**Technikai beállítás és mérés-audit**

Google Analytics, Google Tag Manager, konverziómérés, shopping feedek. Ha sejted, hogy rossz a mérésed, de nem tudod, mit csinálj: az ingyenes hibafeltárás után kapsz árajánlatot a javításra.

---

## Amit ez a fájl nem mutat

- Az aloldalak szövegét (`/kepzes`, `/ugynokseg`, `/arak`, `/rolam`,
  `/eredmenyek`, `/kapcsolat`, `/blog`, jogi oldalak).
- A fejléc és a lábléc szövegét — ezek minden oldalon azonosak.
- Az oldalcímeket és meta-leírásokat, amiket a Google talál meg.

